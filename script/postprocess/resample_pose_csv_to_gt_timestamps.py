...
def load_gt_info(path: Path) -> GroundTruthInfo:
    ...
    if isinstance(payload, dict):
        ...
        timestamp_policy = str(payload.get("timestamp_policy", meta.get("timestamp_policy", "")))
...
def main() -> int:
    ...
    gt_info = load_gt_info(gt_json)
    gt_times_s = gt_info.times_s
    ...
    manifest = {
        ...
        "gt_time_count": int(gt_info.sample_count),
        "gt_timestamp_source": gt_info.timestamp_source,
        "gt_timestamp_policy": gt_info.timestamp_policy,
        "output_pose_count": int(final_times.size),
        "estimate_first_s_before_shift": float(series.times_s[0]),
        "estimate_last_s_before_shift": float(series.times_s[-1]),
        "shifted_estimate_first_s": float(shifted_series.times_s[0]),
        "shifted_estimate_last_s": float(shifted_series.times_s[-1]),
        "gt_first_s": float(gt_times_s[0]),
        "gt_last_s": float(gt_times_s[-1]),
        "gt_duration_s": float(gt_times_s[-1] - gt_times_s[0]) if gt_times_s.size > 1 else 0.0,
        "estimate_duration_s_after_shift": float(shifted_series.times_s[-1] - shifted_series.times_s[0])
        if shifted_series.times_s.size > 1
        else 0.0,
        "overlap_first_s": float(overlap_times[0]),
        "overlap_last_s": float(overlap_times[-1]),
        "overlap_duration_s": float(overlap_times[-1] - overlap_times[0]) if overlap_times.size > 1 else 0.0,
        "output_first_s": float(final_times[0]),
        "output_last_s": float(final_times[-1]),
        "output_duration_s": float(final_times[-1] - final_times[0]) if final_times.size > 1 else 0.0,
        "dropped_gt_samples_outside_overlap": int(gt_times_s.size - overlap_times.size),
        "dropped_gt_samples_by_max_gap": int(overlap_times.size - final_times.size),
    }
