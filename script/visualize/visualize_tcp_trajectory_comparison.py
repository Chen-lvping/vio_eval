...
let mode = "se3";
let activeIndex = 0;
let yaw = inferInitialYaw();
let pitch = inferInitialPitch();
let zoom3d = 1.0;
...
function inferInitialYaw() {
  const fallback = -0.55;
  const refAlgo = data.algorithms && data.algorithms.length ? data.algorithms[0] : null;
  if (!refAlgo || !refAlgo.points || refAlgo.points.length < 2) return fallback;
  const gt = refAlgo.points.map(p => p.gt);
  let dx = gt[gt.length - 1][0] - gt[0][0];
  let dy = gt[gt.length - 1][1] - gt[0][1];
...
function inferInitialPitch() {
  const fallback = 0.42;
  const refAlgo = data.algorithms && data.algorithms.length ? data.algorithms[0] : null;
  if (!refAlgo || !refAlgo.points || !refAlgo.points.length) return fallback;
  const gt = refAlgo.points.map(p => p.gt);
  ...
  const planar = Math.max(maxX - minX, maxY - minY, 1e-6);
  const vertical = maxZ - minZ;
  if (vertical > planar * 0.55) return 0.58;
  return fallback;
}
...
function project3d(p, view, w, h) {
  ...
  const y2 = cp * y1 - sp * z;
  const z2 = sp * y1 + cp * z;
  return [w / 2 + x1 * view.scale, h / 2 - (-z2 + y2 * 0.18) * view.scale];
}
...
function renderMetrics() {
  ...
  for (const [label, key, unit] of rows) {
    if (!data.algorithms.some(algo => Number.isFinite(Number(algo.metrics[key])))) continue;
    const cells = data.algorithms.map(a => {
      const value = Number(a.metrics[key]);
      return `<td>${Number.isFinite(value) ? `${value.toFixed(2)} ${unit}` : "n/a"}</td>`;
    }).join("");
...
def metric_rmse(summary: Dict[str, Dict[str, float]], name: str) -> float:
...
def algo_payload(name: str, color: str, eval_dir: Path, max_points: int, helpers: Dict[str, object]) -> dict:
    ...
    metrics = {
        "ape_translation_se3_rmse_mm": metric_rmse(summary, "ape_translation_se3"),
        "ape_translation_sim3_rmse_mm": metric_rmse(summary, "ape_translation_sim3"),
        "rpe_translation_5cm_rmse_mm": metric_rmse(summary, "rpe_translation_5cm"),
        "rpe_translation_5cm_source_rmse_mm": metric_rmse(summary, "rpe_translation_5cm_source"),
        "ape_rotation_se3_rmse_deg": metric_rmse(summary, "ape_rotation_se3"),
        "rpe_rotation_5cm_rmse_deg": metric_rmse(summary, "rpe_rotation_5cm"),
        "rpe_rotation_5cm_source_rmse_deg": metric_rmse(summary, "rpe_rotation_5cm_source"),
    }
    points = []
    for i in idx:
        points.append(
...
def write_summary(output_dir: Path, algorithms: Iterable[dict]) -> Path:
    ...
    with path.open("w", newline="", encoding="utf-8") as handle:
        fields = [
            ...
            "rpe_translation_5cm_source_rmse_mm",
            "ape_rotation_se3_rmse_deg",
            "rpe_rotation_5cm_rmse_deg",
            "rpe_rotation_5cm_source_rmse_deg",
            "sample_count",
            "duration_s",
        ]
...
def main() -> int:
    ...
    payload = {
        "subtitle": "TCP comparison. Default 3D view follows the dominant motion direction; VIO chain: T_world_tcp = T_world_imu @ inv(T_left_camera_imu) @ inv(T_tcp_left_camera).",
        "inputs": {
